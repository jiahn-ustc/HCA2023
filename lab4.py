...
class Loop_Execution():
    ...
    def run(self):
        #第一步更新邻接矩阵、latency_matrix[i][j]即为第i个与第j个指令之间的延迟数
                ...
                if self.is_FP_ALU_Op(ir1):
                    ...
                    if ir2[0]=="bne":
                        if ir1[1]==ir2[1] or ir1[1] == ir2[2]:
                            self.latency_matrix[i][j]=1
                            #print('1')
        #第二步通过第i、j个指令之间的延迟数与第i、j指令之间的物理距离，来更新第i、i+1之间的延迟数
        for i in range(len(self.instructions)):
            distance = 0
            for j in range(i+1,len(self.instructions)):
...
print("unscheduled:")
loop_execution_unscheduled = Loop_Execution(instructions_unscheduled)
loop_execution_unscheduled.run()
print("scheduled:")
loop_execution_scheduled = Loop_Execution(instructions_scheduled)
loop_execution_scheduled.run()