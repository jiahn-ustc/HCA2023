...
class CPU:
    ...
    def add(self, rd, rs1, rs2):
        self.registers[rd] = self.ALU(
            self.registers[rs1], self.registers[rs2], "add")
        self.pc += 4

    def addi(self, rd, rs1, imm):
        imm = self.imm_expand(imm, "addi")
        self.registers[rd] = self.ALU(self.registers[rs1], imm, "add")
        self.pc += 4

    def slli(self, rd, rs1, imm):
        print("execute slli")
        print("imm=",imm)
        self.registers[rd] = self.ALU(self.registers[rs1], imm, "slli")
        print("4*i=",self.registers[rd])
        self.pc += 4

    def blt(self, rs1, rs2, imm):
        imm = self.imm_expand(imm, "blt")
        r = self.ALU(self.registers[rs1], self.registers[rs2], "blt")
        if r == 1:
            self.pc += (imm << 1)
        else:
            self.pc += 4
    ...
    def lw(self, rd, rs1, imm):
        ...
        address = self.ALU(self.registers[rs1], imm,"add")
        self.registers[rd] = self.read_memory(address)
        self.pc += 4
    ...
    def sw(self, rs2, rs1, imm):
        ...
        self.write_memory(address, self.registers[rs2])
        self.pc += 4
